...
exports.register = (app) => {
    ...
    app.get('/api/variables', async (req, res) => {
        ...
        if (project_id === undefined || typeof project_id !== "number" || name === undefined || typeof name !== 'string')
    ...
    app.post('/api/variables', async (req, res) => {
        ...
        if (project_id === undefined || typeof project_id !== "number" || name === undefined || typeof name !== 'string' || value === undefined)
            res.status(400).json({ error: 'Bad request' });
        try {
            const variables = await variablesDAO.saveVariable(project_id, name, value);
            res.status(200).json(variables);
        } catch (err) {
            res.status(500).json(err);
        }
